...
int Carrier::loseOrder(
	...
	reply = RedisCommand(c2r, "XADD %s NOMKSTREAM * type %u carrierID %lu orderID %lu",
		requestStream, reqstatus::LOSE_ORD_REQ, this->getId(), order_id
	);
	...
	if (reqType != reqstatus::LOSE_ORD_REQ){
	...
	memset(buff, '\0', sizeof(buff)); // LREP_CODE_V
	ReadStreamMsgVal(reply,0,lstMsg, req_formats::LREP_CODE_V, buff);
